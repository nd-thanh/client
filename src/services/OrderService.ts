...
export class OrderService {
    ...
    static async searchorder(formData: string, id: string): Promise<any> {
        try {
            const response = await fetch(`http://localhost:3000/api/order/searchz/${id}`, {
                method: 'POST',
                headers: {
        ...
        static async searchorderByUser(formData: string,id:string): Promise<any> {
            try {
                const response = await fetch(`http://localhost:3000/api/order/search?user=${id}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },

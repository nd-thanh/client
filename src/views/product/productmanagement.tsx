...
export default function ProductManagement() {
    ...
    const showData = async () => {
        try {
            const data: any = await ProductController.fetchProductsAdmin();
    ...
    const showdataCategory = async () => {
        try {
            const res = await CategoryController.getAllCategories(); // Đảm bảo bạn đang sử dụng await ở đây
    ...
    onClick={() =>
        handleHide(
            product.ProductID,
            product.status.trim().toUpperCase() === 'ACTIVE' ? 'SHUTDOWN' : 'ACTIVE'
        )
    }
    colorScheme={product.status.trim().toUpperCase() === 'ACTIVE' ? 'blue' : 'red'}
    size="sm"
    width="100%"
    borderRadius="md"
>
    <i
        className={`fa-solid ${
            product.status.trim().toUpperCase() === 'ACTIVE'
                ? 'fa-eye-slash'
                : 'fa-eye'
        }`}
    ></i>
    {product.status.trim().toUpperCase() === 'ACTIVE' ? 'Ngừng hoạt động' : 'Kích hoạt sản phẩm'}
</Button>
    ...
    >
        <i className="fa-solid fa-trash"></i> Xóa
                                    ...
                                    <Td>
                                        <Badge colorScheme={product.status.trim().toUpperCase() === 'SHUTDOWN' ? "red" : "green"}>
                                            {product.status.trim().toUpperCase() === 'SHUTDOWN' ? "Đang Ẩn" : "Đang Hoạt Động"}
                                        </Badge>
                                    </Td>

...
const AddressInfo = () => {
  ...
  const handleSaveAddress = () => {
    if (isUpdating) {
      ...
      fetch(`${apiUrl}api/address/update`, {
        method: 'POST',
        headers: {
      ...
      // Logic for updating address (pass the address data to your update function)
      // Call your update address function here
    } else {
  ...
  const showDataAddress = async () => {
    try {
      ...
      const decodedToken = jwtDecode(tokenUser); // Giải mã token
      console.log('giá trị id ' + decodedToken.userId)
      const response = await fetch(`${apiUrl}api/address/byuser`, {
        ...
        body: JSON.stringify({
          UserID: decodedToken.userId,  // Pass the UserID in the request body
        }),
      });
  ...
  const [isUpdating, setIsUpdating] = useState(false); // Flag to track if we are updating an existing address
  ...
  return (
    <div className="account-card">
      ...
      >
        {isUpdating ? 'Cập nhật thông tin' : 'Lưu địa chỉ'}
      </button>
            </div>
          </form>
        ) : (

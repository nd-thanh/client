...
export default function UserManagementView() {
    ...
    const handleStatusChange = async (user: any) => {
        try {
            console.log("gia tri"+user.status)
            ...
            const res=  await UserController.updateStatusByID(user.UserID, Status);
            if(res){
    ...
    const [addressByUser,setaddressByUser]=useState<any>([]);
...
>
  {user.status === 'Active' ? 'Block' : 'Unblock'}
      ...
      <Td>
  <Badge colorScheme={user.status.toLowerCase() === 'active' ? 'green' : 'red'}>
    {user.status.toLowerCase() === 'active' ? 'Hoạt động' : 'Bị chặn'}
  </Badge>
</Td>
...
Xem Địa Chỉ
  ...
  <Modal isOpen={isUpdateSuccess} onClose={() => setIsUpdateSuccess(false)}>
    ...
    <ModalOverlay />
    <ModalContent>
        <ModalHeader>Tạo Người Dùng</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
